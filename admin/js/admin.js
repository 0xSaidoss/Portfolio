...
function addNotif(type, message, sub) {
  const notifs = loadNotifs();
  notifs.unshift({
    id:   Date.now().toString(),
    type,          // 'comment' | 'like' | 'visit'
    message,       // texte principal
    sub:  sub||'', // sous-titre (nom projet/article)
    read: false,
    time: new Date().toISOString()
  });
  saveNotifs(notifs.slice(0, 60));
  updateBellBadge();
  // Flash du badge
...
const NOTIF_ICONS = { comment:'💬', like:'♥', visit:'👁️', reply:'↩' };
...
function renderNotifPanel() {
  ...
  const notifs = loadNotifs();
  if (!notifs.length) {
    list.innerHTML = `<div style="padding:2rem 1rem;text-align:center;font-size:.82rem;color:var(--txt3)">
      Aucune notification pour l'instant.<br>
      <span style="font-size:.72rem;opacity:.6">Les likes, commentaires et visites apparaîtront ici.</span>
    </div>`;
    return;
  }
  list.innerHTML = notifs.map(n => `
    <div class="notif-item ${n.read?'':'unread'}">
      ...
      <div class="notif-body">
        <strong>${escHtml(n.message)}</strong>
        ${n.sub ? `<span>${escHtml(n.sub)}</span>` : ''}
      </div>
...
function startNotifWatcher() {
  ...
  let _snap = {};   // { [projectId]: { likes, comments } }
  let _ready = false;
    ...
    try {
      ...
      for (const item of all) {
        const [likes, comments] = await Promise.all([
          Store.getLikes(item.id),
          Store.getComments(item.id),
        ]);
        const prev = _snap[item.id];
        ...
        if (prev) {
          ...
          if (likes > prev.likes) {
            ...
            addNotif('like',
              `${diff} nouveau${diff>1?'x':''} like${diff>1?'s':''}`,
              item.title
            );
          }
          // ── Nouveau commentaire ──
          const newCmts = comments.length - prev.comments;
          if (newCmts > 0) {
          ...
          // ── Nouvelles réponses ──
          const prevReplies = prev.replies || 0;
          const curReplies  = comments.reduce((a,c)=>a+(c.replies||[]).length, 0);
          if (curReplies > prevReplies) {
            addNotif('reply',
              'Nouvelle réponse à un commentaire',
              item.title
            );
          }
          _snap[item.id].replies = curReplies;
        }

        _snap[item.id] = {
          likes,
          comments: comments.length,
          replies:  comments.reduce((a,c)=>a+(c.replies||[]).length, 0),
        };
      }

      // ── Nouvelles visites (vérifie toutes les 2 min) ──
      if (_ready) {
        const visits = await Store.getVisits();
        const today  = new Date().toISOString().slice(0,10);
        const todayV = (visits.daily||{})[today] || 0;
        if (_snap._todayVisits !== undefined && todayV > _snap._todayVisits) {
          addNotif('visit',
            `${todayV} visite${todayV>1?'s':''} aujourd'hui`,
            `Total : ${visits.total || 0} visites`
          );
        }
        _snap._todayVisits = todayV;
      }
      _ready = true;
  ...
  // Premier check pour initialiser les valeurs de référence
  checkUpdates().then(() => {
    setInterval(checkUpdates, 30000); // Poll toutes les 30s
  });
}
    ...
    bellWrap.innerHTML = `
      <div class="notif-panel" id="notifPanel">
        <div class="notif-header">
          <h4>🔔 Notifications</h4>
          <button class="notif-mark-read" id="markReadBtn">Tout lire</button>
        </div>
...
async function viewDashboard() {
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.75rem">
    <p class="dash-recent-title">// Visites <span style="font-family:var(--mono);font-size:.65rem;color:var(--txt3)">— 1 visite = 1 visiteur unique/jour</span></p>
    <button class="btn btn-ghost btn-sm" onclick="resetVisits()" style="font-size:.65rem">↺ Réinitialiser</button>
  </div>
...
function initSettingsEvents() {
  ...
  document.getElementById('exportFullBtn')?.addEventListener('click', async () => {
    ...
    btn.innerHTML = '<span>⟳</span> Export en cours...'; btn.disabled = true;
    try {
      const [projects, posts, arsenal, profile, visits] = await Promise.all([
        Store.getProjects(),
        Store.getPosts(),
        Store.getArsenal(),
        Store.getProfile(),
      ...
      // Export commentaires pour chaque projet + post
      const allItems = [...projects, ...posts];
      const comments = {};
      for (const item of allItems) {
        const c = await Store.getComments(item.id);
        if (c.length) comments[item.id] = c;
      }
      const data = {
        version: '2.0',
        exported: new Date().toISOString(),
        projects, posts, arsenal, profile, visits, comments,
        meta: {
          totalProjects: projects.length,
          totalPosts: posts.length,
          totalComments: Object.values(comments).reduce((a,c)=>a+c.length,0),
        }
      };
      downloadJSON(data, `saidoss-backup-complet-${new Date().toISOString().slice(0,10)}.json`);
      showToast('✓ Export complet téléchargé !');
    } catch(e) { showToast('✗ Erreur export : ' + e.message); }
    btn.innerHTML = '<span>↓</span> Exporter tout (JSON complet)'; btn.disabled = false;
  });

  // ── Export LITE (projets + posts seulement) ──
  document.getElementById('exportLiteBtn')?.addEventListener('click', async () => {
    const btn = document.getElementById('exportLiteBtn');
    btn.innerHTML = '<span>⟳</span> Export...'; btn.disabled = true;
    try {
      const [projects, posts] = await Promise.all([Store.getProjects(), Store.getPosts()]);
      const data = { version: '2.0', exported: new Date().toISOString(), projects, posts };
      downloadJSON(data, `saidoss-backup-lite-${new Date().toISOString().slice(0,10)}.json`);
      showToast('✓ Export téléchargé.');
    } catch(e) { showToast('✗ Erreur : ' + e.message); }
    btn.innerHTML = '<span>↓</span> Exporter projets + articles seulement'; btn.disabled = false;
  });

  // ── Import JSON (complet ou lite) ──
  document.getElementById('importFile')?.addEventListener('change', e => {
    ...
    reader.onload = async ev => {
      try {
        ...
        msg.style.display = 'block';
        msg.style.background = 'rgba(79,156,249,.06)';
        msg.style.border = '1px solid rgba(79,156,249,.2)';
        ...
        if (d.arsenal?.length) {
          for (const a of d.arsenal) { await Store.upsertArsenalCat(a); count++; }
        }
        if (d.profile) {
          await Store.saveProfile(d.profile); count++;
        }
        if (d.comments) {
          for (const [itemId, cmts] of Object.entries(d.comments)) {
            for (const cm of cmts) { await Store.addComment(itemId, cm); count++; }
          }
        }

        msg.style.background = 'rgba(34,197,94,.06)';
        msg.style.border = '1px solid rgba(34,197,94,.2)';
        msg.style.color = 'var(--green)';
        msg.textContent = '✓ ' + count + ' éléments restaurés dans Firebase.';
        showToast('✓ Import complet réussi !');
        renderView('dashboard');
      } catch(err) {
        msg.style.display = 'block';
        ...
        msg.textContent = '✗ Fichier invalide : ' + err.message;
      }
    };
      ...
      <div style="display:flex;flex-direction:column;gap:.3rem;margin-top:.25rem">
        ${(cat.skills||[]).map(s => `
          <div style="display:flex;align-items:center;gap:.5rem;padding:.2rem 0">
            <span style="width:5px;height:5px;border-radius:50%;background:var(--accent);opacity:.7;flex-shrink:0"></span>
            <span style="font-size:.82rem;color:var(--txt2)">${esc(s.name)}</span>
          </div>`).join('')}
      </div>
...
function renderArsenalEditor() {
  ...
  <div style="display:flex;gap:.75rem;margin-bottom:1rem">
    <div class="field" style="flex:0 0 80px">
      ...
      <input id="arsCatIcon" class="input" value="${esc(c.icon||'⬡')}" placeholder="💀"/>
    </div>
  ...
  <div id="arsSkillsList" style="display:flex;flex-direction:column;gap:.4rem;margin-bottom:1rem">
    ${c.skills.map((s,i) => `
    <div style="display:flex;gap:.5rem;align-items:center">
      <input class="input" value="${esc(s.name)}" placeholder="Nom compétence" oninput="updateArsenalSkill(${i},'name',this.value)" style="flex:1"/>
      <button class="btn btn-sm btn-red" onclick="removeArsenalSkill(${i})" style="flex-shrink:0">✕</button>
    </div>`).join('')}
  </div>
...
async function resetVisits() {
  if (!confirm('Réinitialiser toutes les statistiques de visites ?\nLes anciennes données seront perdues.')) return;
  try {
    await Store.resetVisits();
    renderView('dashboard');
